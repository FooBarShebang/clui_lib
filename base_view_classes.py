...
class CLUI_ABC(abc.ABC):
    ...
    methods clear() and show() are abstract (virtual) - must be implemented by
    the sub-classes.
    ...
    def show(self) -> None:
        """
        Virtual method to print the current graphical representation of a
        widget. Not implemenented - the non-abstract sub-classes must implement
        this method as non-virtual.
...
class HWidget_ABC(CLUI_ABC):
    ...
    show() is abstract (virtual) - must be implemented by the sub-classes.
...
class TextLabel(HWidget_ABC):
    ...
    def show(self) -> None:
        """
        Prints out the currently stored string with at least one space after it
...
class BarControl_ABC(HWidget_ABC):
    ...
    to a float in the range 0.0 to 1.0 inclusively. Method show() is not yet
    implemented - must be done by the sub-classes.
...
class Slider(BarControl_ABC):
    ...
    def show(self) -> None:
        """
        Prints out the representation of the current state of the widget.
...
class HContainer(CLUI_ABC):
    ...
    def show(self) -> None:
        """
        Prints the graphical representations of all stacked widgets (their inner
        states) in a single line string.
